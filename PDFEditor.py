...
def custom_messagebox(parent, title, message, typ="info"):
    ...
    Args:
        parent: Okno nadrzędne (główne okno aplikacji)
        title: Tytuł okna dialogowego
        message: Treść komunikatu
        typ: Typ komunikatu - "info", "error", "warning", "question", "yesnocancel"
    
    Returns:
        - None dla info, error, warning
        - True/False dla question (Yes/No)
        - True/False/None dla yesnocancel (Yes/No/Cancel)
    """
    ...
    icon = icons.get(typ, "ℹ️")
    
    # Ramka główna
    main_frame = ttk.Frame(dialog, padding="20")
    main_frame.pack(fill="both", expand=True)
    
    # Ramka z ikoną i tekstem
    content_frame = ttk.Frame(main_frame)
    content_frame.pack(fill="both", expand=True, pady=(0, 20))
    
    # Ikona
    icon_label = tk.Label(content_frame, text=icon, font=("Arial", 32), bg=bg_color, width=2)
    icon_label.pack(side="left", padx=(0, 15), pady=10)
    
    # Tekst komunikatu
    msg_label = tk.Label(content_frame, text=message, justify="left", wraplength=400, font=("Arial", 10))
    msg_label.pack(side="left", fill="both", expand=True, pady=10)
    ...
    if typ == "question":
        ...
        # Przyciski Yes/No/Cancel dla pytania z anulowaniem
        yes_btn = ttk.Button(button_frame, text="Tak", command=on_yes, width=10)
        yes_btn.pack(side="left", padx=5)
        no_btn = ttk.Button(button_frame, text="Nie", command=on_no, width=10)
        no_btn.pack(side="left", padx=5)
        ...
        # Przycisk OK dla pozostałych typów
